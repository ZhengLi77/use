...
谢老师服务器已保存了一个安全帽检测的数据集，可用

```shell
cp /data/alist/file/GDUT-HWD.zip ./
```
...
```shell
unzip -q GDUT-HWD.zip -d ./dataset
```

![image-20241021105827856](./assets/image-20241021105827856.png)

可以把这个数据集下载到本地电脑上查看

在数据集文件夹labels下新建一个txt文件，文件输入以下信息

![image-20241021131053055](./assets/image-20241021131053055.png)
...
安装目标检测常用的标注软件，输入labelimg打开这个软件

![image-20241021131304872](./assets/image-20241021131304872.png)

opendir---选择数据集的JPEGImages文件夹---changesavedir---选择数据集的labels文件夹

![image-20241021131326545](./assets/image-20241021131326545.png)

![image-20241021131544506](./assets/image-20241021131544506.png)

这样我们便看到了数据集下的标签文件（yolo类型）具体标注的是什么信息了，明显可见一共5个类别，对应安全帽的佩戴情况，接下来就可以进行迁移训练了。

#### 迁移训练

首先需要数据集划分：训练集和验证集（正规也需要测试集，但我们不需要测试模型准确度，故不划分测试集了）。
